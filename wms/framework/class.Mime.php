...
class MIME_mail {
	...
	private $base64_func= '';		# if !specified use PHP's base64  
	private $qp_func = '';      	# None at this time
	...
	// Constructor.
	function __construct($from="", $to="", $subject="", $body="", $headers = "") {
		$this->to = $to;
		$this->from = $from;
	...
	function fattach($path, $description = "", $contenttype = OCTET, $encoding = BASE64, $disp = '') {
		...
		return $this->attach($data, $description, $contenttype, $encoding, $disp);	
	}
